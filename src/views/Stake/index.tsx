...
function Stake() {
    ...
    const jazzBalance = useSelector<IReduxState, string>(state => {
        return state.account.balances && state.account.balances.jazz;
    });
    ...
    const stakeAllowance = useSelector<IReduxState, number>(state => {
        return state.account.staking && state.account.staking.jazz;
    });
    const unstakeAllowance = useSelector<IReduxState, number>(state => {
        return state.account.staking && state.account.staking.blue;
    });
    ...
    const setMax = () => {
        if (view === 0) {
            setQuantity(jazzBalance);
        } else {
            setQuantity(blueBalance);
        }
    };
    ...
    const hasAllowance = useCallback(
        token => {
            if (token === "jazz") return stakeAllowance > 0;
            if (token === "blue") return unstakeAllowance > 0;
            return 0;
        },
        [stakeAllowance],
    );
                    ...
                    <Grid className="stake-card-grid" container direction="column" spacing={2}>
                        <Grid item>
                            <div className="stake-card-header">
                                <p className="stake-card-header-title">JAZZ Staking (🎷, 🎷)</p>
                                <RebaseTimer />
                            </div>
                        </Grid>
                                ...
                                <Grid container spacing={2}>
                                    ...
                                    <Grid item xs={6} sm={4} md={4} lg={4}>
                                        <div className="stake-card-index">
                                            ...
                                            <p className="stake-card-metrics-value">{currentIndex ? <>{trim(Number(currentIndex), 2)} JAZZ</> : <Skeleton width="150px" />}</p>
                                        </div>
                                    </Grid>
                                </Grid>
                            </div>
                        ...
                        <div className="stake-card-area">
                            {!address && (
                                <div className="stake-card-wallet-notification">
                                    ...
                                    <p className="stake-card-wallet-desc-text">Connect your wallet to stake JAZZ tokens!</p>
                                </div>
                            )}
                            {address && (
                                <div>
                                    <div className="stake-card-action-area">
                                            ...
                                            {view === 0 && (
                                                <div className="stake-card-tab-panel">
                                                    {address && hasAllowance("jazz") ? (
                                                        ...
                                                        >
                                                            <p>{txnButtonText(pendingTransactions, "staking", "Stake JAZZ")}</p>
                                                        </div>
                                                    ) : (
                                                        <div
                                                            className="stake-card-tab-panel-btn"
                                            ...
                                            {view === 1 && (
                                                <div className="stake-card-tab-panel">
                                                    {address && hasAllowance("blue") ? (
                                                        ...
                                                        >
                                                            <p>{txnButtonText(pendingTransactions, "unstaking", "Unstake JAZZ")}</p>
                                                        </div>
                                                    ) : (
                                                        <div
                                                            className="stake-card-tab-panel-btn"
                                        ...
                                        <div className="stake-card-action-help-text">
                                            {address && ((!hasAllowance("jazz") && view === 0) || (!hasAllowance("blue") && view === 1)) && (
                                                <p>
                                    ...
                                    <div className="stake-user-data">
                                        ...
                                        <div className="data-row">
                                            ...
                                            <p className="data-row-value">{isAppLoading ? <Skeleton width="80px" /> : <>{nextRewardValue} BLUE</>}</p>
                                        </div>
